...
evpi_private_garden_data <- mcSimulation_table[c(1:77, 81:85)] # only public garden related
# library(dplyr) # Move "NPV" to the last position using dplyr's select()
evpi_private_garden_data <- evpi_private_garden_data %>% select(-NPV_garden, NPV_garden)

evpi_private_STEM_data <- mcSimulation_table[c(1:76, 78, 81:85)] # only public STEM related
evpi_private_STEM_data <- evpi_private_STEM_data %>% select(-NPV_garden_STEM, NPV_garden_STEM)

evpi_public_garden_data <- mcSimulation_table[c(1:76, 79, 81:85)] # only public garden related
evpi_public_garden_data <- evpi_public_garden_data %>% select(-NPV_garden_public_school, NPV_garden_public_school)

evpi_public_STEM_data <- mcSimulation_table[c(1:76, 80:85)]  # only public STEM related
evpi_public_STEM_data <- evpi_public_STEM_data %>% select(-NPV_garden_STEM_public_school, NPV_garden_STEM_public_school)
...
evpi_test_public_STEM <- multi_EVPI_test(mc = evpi_private_garden_data, first_out_var = "NPV_garden")
...
plot_evpi_garden <- plot_evpi(EVPIresults = evpi_test_public_STEM, 
